...
class txtweb_innoz_bridge {
    ...
    private static $message // the sms sent to the app
		...
		,$queryParams//params send via query
		;
	...
	function initBridge() {
		...
		$platformDetails = array(
		    'Txtweb' =>array('textMessage' => 'txtweb-message','mobHash'=>'txtweb-mobile' ,'userLocation'=>'txtweb-location','appId'=>'app'),
			'Innoz' =>array('textMessage'=>'message','mobHash'=>'mobile','userLocation'=>'region','appId'=>'key')
		); 
		self::$platformDetails = $platformDetails;
		
		
		$requestToImplemetationMapping = array('Txtweb' => 'Innoz','Innoz'=>'Txtweb'); //platform mapping
	...
	private function mapParams($params) {/*maps parameters sent to the requested platform to those of the implemented platform*/
	...
	private  function fetchResult() {//fetches result from original implementation
	    self::$result = self::send(self::$requestUrl,self::$queryParams,'GET');
    ...
    private function send( $url, $data ,$method = 'GET') //sends data array(param=>val,...) to the page $url in post method and returns the reply string
    {
